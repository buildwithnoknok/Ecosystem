...
- `noknok_Mezzanine_4Cluster` (symbol) + `noknok_Mezzanine_4Cluster_Pads` (flat pads, hub bottom, origin on hole H1) / `noknok_Mezzanine_4Cluster_PogoPins_BWCD` (spring pins, mezzanine top, origin on its H1) — the PicoHub-to-mezzanine board-to-board interface, rev 2 (frozen 22 Sep 2026): 24 contacts on 3.5 mm pitch in four clusters, each beside a mounting hole and each a complete port on its own, so a mezzanine populates only what it uses. **A** (2×3): VBUS_PD ×4 + GND + MEZZ_EN — raw 5–20 V for boards with their own regulator. **D** (2×3): USB D±, GND, MEZZ_5V, MEZZ_PRESENT, MEZZ_FAULT — a USB port. **B** (2×4): SPI + IRQ + AIN + GND + MEZZ_5V — an SPI port. **C** (2×2): +3V3_A, GND, SDA_A, SCL_A — a Qwiic port. MEZZ_5V = switched 5 V (SY6280, 1.45 A) enabled by MEZZ_EN; PRESENT low = "I take the USB uplink". Pad numbers A1…C4, pin names = nets; coordinates and rules in the MOD-0011 spec §6. Spring pin: BAT WIRELESS BWCD-L5.5W2.0H3.0 (LCSC C2826550), 4.5 mm board-to-board on M2.5 × 4.5 standoffs. (Rev 1, `noknok_Mezzanine_2x12`, was removed before any board used it.)
